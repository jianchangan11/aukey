...
'''114-4184958-7021835 此订单在自发货系统的sku和之前下载的新数据_周里面的sku不一致'''
#读取产品表，区分成品和配件sku
...
#选出补寄订单中sku为成品的订单，并统计每个订单下sku的数量
...
#选出每个成品sku订单的补寄时间，并和数量merge到一张表
...
#保留最新的配件补寄订单时间
origin_zifa_bp_num.drop_duplicates(subset=['平台订单号','账号'],keep='last',inplace=True)
print(len(origin_zifa_bp))
origin_zifa_bp.head(2)
#统计每个补寄配件的次数，配件补寄次数是按照包裹号统计的，几个包裹号，就补寄几次
...
#将重复的补寄订单时间去掉，使得补寄次数和补寄订单时间逐行对应
...
order_l =buji_sku_merge[buji_sku_merge['平台订单号'].isin(walmart['Order#'])]
for i in range(len(walmart)):
    if walmart.iat[i, 1] in order_l['平台订单号'].tolist():
        walmart_sit = walmart['Order#'].tolist().index(walmart.iat[i,1])
        buji_sku_merge.loc[buji_sku_merge['平台订单号'] ==walmart.iat[i,1],'平台订单号' ]= walmart.loc[walmart_sit,'PO#']


# 量太大 先存一下
buji_sku1=buji_sku_merge.drop('订单类型', axis=1)
