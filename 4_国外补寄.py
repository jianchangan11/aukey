...
#创建副本，填充nan值，汇总每个订单的补寄数量
inventory_buji = inventory.copy()
...
inventory_buji.head()
#选出国外补寄新件，国内补寄退件，国内补寄破损件，国外补寄配件
...
origin_buji.to_excel(excel_writer="D:/work/origin_buji/py" + now + r"origin_buji——new.xlsx", index=False, encoding='utf-8')
...
#总的补寄数量不同以及订单登记错误(这个代码运行可能比较慢，后期可以尝试优化)
...
# 加个去重！！！！！
now = time.strftime("%Y-%m-%d-%H_%M",time.localtime(time.time()))
dengji_buji.drop_duplicates(subset=["订单号"],keep='first',inplace= True)
...
inventory_list =inventory_3['平台订单号'].drop_duplicates()  #去重，避免merge出现的数据倍化
...
#读取售后人员，和迭代表和匹配表的结果进行merge，选出sku的最新负责人
...
inventory_in_buji12 = inventory_in_buji12[(inventory_in_buji12['补寄sku订单时间_y']>inventory_in_buji12['开始时间']) & (inventory_in_buji12['补寄sku订单时间_y']<inventory_in_buji12['结束时间'])]

#去掉已校对-P，已校对-check的订单
now = time.strftime("%Y-%m-%d-%H_%M",time.localtime(time.time()))
...
'''
9863022123122 ,112-7175907-1789840,112-0969692-0156225此订单无误，
在输出的漏登excel中要把对应记录删除
'''
inventory_in_buji12 = inventory_in_buji12[~(inventory_in_buji12['平台订单号'] == '112-0969692-0156225')&(inventory_in_buji12['平台订单号'] == '9863022123122')&(inventory_in_buji12['平台订单号'] == '112-7175907-1789840')]
